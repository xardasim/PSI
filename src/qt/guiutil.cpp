...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("PolishSexIndustry"))
        return false;
...
{
    // Convert PolishSexIndustry:// to PolishSexIndustry:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("PolishSexIndustry://"))
    {
        uri.replace(0, 12, "PolishSexIndustry:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "PolishSexIndustry.lnk";
}
...
{
    return GetAutostartDir() / "PolishSexIndustry.desktop";
}

bool GetStartOnSystemStartup()
{
    boost::filesystem::ifstream PolishSexIndustryFile(GetAutostartFilePath());
    if (!PolishSexIndustryFile.good())
        return false;
    // Scan through file for "Hidden=true":
    std::string line;
    while (!PolishSexIndustryFile.eof())
    {
        getline(PolishSexIndustryFile, line);
        if (line.find("Hidden") != std::string::npos &&
            line.find("true") != std::string::npos)
            return false;
    }
    PolishSexIndustryFile.close();
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        boost::filesystem::ofstream PolishSexIndustryFile(GetAutostartFilePath(), std::ios_base::out|std::ios_base::trunc);
        if (!PolishSexIndustryFile.good())
            return false;
        // Write a bitcoin.desktop file to the autostart directory:
        PolishSexIndustryFile << "[Desktop Entry]\n";
        PolishSexIndustryFile << "Type=Application\n";
        PolishSexIndustryFile << "Name=PolishSexIndustry\n";
        PolishSexIndustryFile << "Exec=" << pszExePath << " -min\n";
        PolishSexIndustryFile << "Terminal=false\n";
        PolishSexIndustryFile << "Hidden=false\n";
        PolishSexIndustryFile.close();
    }
    return true;
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("PolishSexIndustry-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  PolishSexIndustry-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("PolishSexIndustry-Qt"));
