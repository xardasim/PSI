...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizePSIAmount"))
        settings.setValue("nAnonymizePSIAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizePSIAmount = settings.value("nAnonymizePSIAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizePSIAmount"))
        SoftSetArg("-anonymizePolishSexIndustryamount", settings.value("nAnonymizePSIAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizePSIAmount:
            nAnonymizePSIAmount = value.toInt();
            settings.setValue("nAnonymizePSIAmount", nAnonymizePSIAmount);
            emit anonymizePSIAmountChanged(nAnonymizePSIAmount);
            break;
        default:
            break;
        }
