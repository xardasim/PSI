...
void Shutdown()
{
    ...
    RenameThread("PolishSexIndustry-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: PolishSexIndustry.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: PolishSexIndustryd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizePolishSexIndustryamount=<n> " + _("Keep N PSI anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. PolishSexIndustry is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. PolishSexIndustry is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("PolishSexIndustry version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "PolishSexIndustry server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of PolishSexIndustry") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart PolishSexIndustry to complete") << "\n";
    ...
    nAnonymizePSIAmount = GetArg("-anonymizeros2coinamount", 0);
    if(nAnonymizePSIAmount > 999999) nAnonymizePSIAmount = 999999;
    if(nAnonymizePSIAmount < 2) nAnonymizePSIAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizePSIAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize PSI Amount %d\n", nAnonymizePSIAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1PSI+1000 == (.1PSI+100)*10
       10PSI+10000 == (1PSI+1000)*10
    */
