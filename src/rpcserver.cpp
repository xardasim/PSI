...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "Stop PolishSexIndustry server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "PolishSexIndustry server stopping";
}
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        string strWhatAmI = "To use PolishSexIndustryd";
        if (mapArgs.count("-server"))
            strWhatAmI = strprintf(_("To use the %s PolishSexIndustry"), "\"-server\"");
        else if (mapArgs.count("-daemon"))
            strWhatAmI = strprintf(_("To use the %s PolishSexIndustry"), "\"-daemon\"");
