    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat PolishSexIndustry and make it unusable. To
...
bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizePSIAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
//        }
    ...
    if(!sessionFoundMasternode){
        ...
        double fPSISubmitted = (sessionTotalValue / CENT);
        LogPrintf("Submitting Darksend for %f PSI CENT - sessionTotalValue %d\n", fPSISubmitted, sessionTotalValue);
...
void CDarkSendPool::GetDenominationsToString(int nDenom, std::string& strDenom){
    ...
    //
    // bit 0 - 100PSI+1 ( bit on if present )
    // bit 1 - 10PSI+1
    // bit 2 - 1PSI+1
    // bit 3 - .1PSI+1
    // bit 3 - non-denom
...
void ThreadCheckDarkSendPool()
{
    ...
    RenameThread("PolishSexIndustry-darksend");
        ...
        if(c % (60*5)*(nLiquidityProvider+1) == 0){
            if(nLiquidityProvider!=0){
                ...
                if(nRand == 50+nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8){
                    ...
                    nAnonymizePSIAmount = (rand() % nLeftToAnon)+3;
                } else {
